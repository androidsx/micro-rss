 ...
 */
public abstract class AbstractUpdateService extends Service implements Runnable {
     ...
     * update service wakes up, checks whether any widget needs updating, sets a
     * new alarm, and sleeps again.
     */
    ...
    private static void requestUpdate(int[] appWidgetIds) {
        Log.i(TAG, "Request update for widgets [" + arr2str(appWidgetIds) + "] (just queued them up)");
        synchronized (sLock) {
     ...
     * <p>
     * Services in Android run in the main thread of the application, so it is important
     * to create a new thread for an expensive operation such as this one.
     */
    @Override
    public void run() {
        ...
        while (hasMoreUpdates()) {
            ...
            try {
              ...
              Log.e(TAG, "Exception while processing content for the widget "
                      + appWidgetId + ". We'll end up in the error view.", e);                
              ...
              FlurryAgent.onError(FlurryConstants.ERROR_ID_UPDATE_SERVICE, "Update service fails unexpectedly", e.getClass().toString());
            }
        ...
        if (areThereHumans) {
          ...
          alarmManager.set(AlarmManager.RTC_WAKEUP,
              timeForNextUpdateAlarm(UPDATE_INTERVAL), pendingIntent);
        } else {
    ...
    private long timeForNextUpdateAlarm(long minUpdateIntervalMillis) {
      ...
      Log.i(TAG, "Requesting next update in "
            + (nextUpdate - nowMillis) / DateUtils.MINUTE_IN_MILLIS
            + " minutes");
    ...
    /**
     * Computes the throttle: if we calculated an update too quickly in the
     * future, wait this interval and try rescheduling.
     */
